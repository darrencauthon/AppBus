...
SOLUTION_FILE = "\"#{SOLUTION_ROOT}\\src\\AppBus.sln\""

BIN_DIRECTORY = "#{SOLUTION_ROOT}\\src\\AppBus\\bin\\Release"

DEPLOY_DIRECTORY = "#{SOLUTION_ROOT}\\deploy"
DEPLOY_FILE =  "#{DEPLOY_DIRECTORY}\\AppBus.dll"
...
desc "Create the AppBus.dll in /deploy"
...
task :clean_deploy do
	File.delete "#{DEPLOY_DIRECTORY + "/AppBus.dll"}" if File.exists?(DEPLOY_DIRECTORY + "/AppBus.dll")
	File.delete "#{DEPLOY_DIRECTORY + "/AppBus.pdb"}" if File.exists?(DEPLOY_DIRECTORY + "/AppBus.pdb")
